...
ライブのプロセスビューでシステム上で動作しているものを確認できます：プロセスごとの CPU・メモリ・ディスク読み書きレート、プロセスツリー、アプリの関連付け。プロセスの終了はガード付きフローで行い、選択項目をリスク別にグループ化して表示します。重要なシステムプロセスは常に保護され、他のユーザーが所有するプロセスは理由を示して拒否され、すべての操作は事前に確認のうえ操作履歴に記録されます。内蔵の AI チャットは、見慣れないプロセスの説明や「なぜディスクが忙しいの？」といったリアルタイムの質問に、同じ型付きデータを使って答えます。接続には、ローカルにインストール済みの認証済み AI プロバイダ CLI を使います。
...
- **既定は読み取り専用**：スキャン、ディスク分析、大容量ファイル・重複ファイルの検出、ライブのプロセス一覧、操作履歴。変更系ツール（クリーンアップ、完全削除、アンインストール、プロセス終了、スタートアップ項目、システム設定）は `--enable-mutations` 付きで起動しない限り拒否されます。
