...
通过实时进程视图查看系统正在运行的内容：每个进程的 CPU、内存和磁盘读写速率、进程树以及应用关联。结束进程使用受保护的流程，按风险分组显示所选内容——关键系统进程始终受保护，其他用户拥有的进程会被拒绝并说明原因，每个操作都需先确认并记录到操作历史。内置 AI 对话可以解释不熟悉的进程，并使用相同的类型化数据回答"我的磁盘为什么在忙？"等实时问题——通过你本地已安装且已登录的 AI 服务商 CLI 连接。
...
- **默认只读**：扫描、磁盘分析、大文件与重复文件发现、实时进程清单、操作历史。变更类工具（清理、永久删除、卸载、结束进程、启动项、系统设置）在未使用 `--enable-mutations` 启动服务器时会直接拒绝执行。
